...
/// Reading an export that lists accounts as labelled fields.
///
...
@Suite("Labelled text import")
struct LabelledTextImportTests {
    ...
    private var fixture: String {
        get throws {
            ...
            let file = here.appendingPathComponent("Fixtures/LabelledTextExport.rtf")
    ...
    func readsTheFixture() throws {
        let result = LabelledTextImport.read(try fixture)
    ...
    func splitsOnLineSeparators() throws {
        let issuers = LabelledTextImport.read(try fixture).accounts.map(\.account.issuer)
    ...
    func mapsColours() throws {
        let accounts = LabelledTextImport.read(try fixture).accounts
    ...
    // MARK: - Files that are not a labelled export

    @Test("An unrelated file yields nothing rather than failing")
    func ignoresUnrelatedFiles() {
        #expect(LabelledTextImport.read("hello, world").isEmpty)
        #expect(LabelledTextImport.read("").isEmpty)
        #expect(LabelledTextImport.read("{\\rtf1\\ansi nothing to see}").isEmpty)
    }

    /// A file in another language is the known limitation of reading a document written for
    /// humans.
    ...
    func ignoresLocalisedExports() {
        let french = """
            ...
            AAAAAAAAAAAAAAAA\\u8232 }
            """
        #expect(LabelledTextImport.read(french).isEmpty)
    }
    ...
    func refusesMissingSecret() {
        ...
        let result = LabelledTextImport.read(rtf)
    ...
    /// The defect this reader shipped with. Absent settings were filled in with sha1, 6
    /// and 30, so if a future export renamed a label, an eight digit account would import
    /// as six and generate codes rejected forever, found at a login rather than here.
    ///
    /// A file of this shape always writes every field, so an absent one means the parse
    /// failed. That is the opposite of the `otpauth://` parser, where the specification
    /// says an absent parameter means the default.
    @Test(
        "A missing code affecting setting is refused, never defaulted",
        arguments: [
            ("Digits: 6", ImportRefusal.Reason.missingSetting(.period)),
            ("Period: 30 seconds", ImportRefusal.Reason.missingSetting(.digits)),
        ]
    )
    func refusesMissingSettings(present: String, expected: ImportRefusal.Reason) {
        let rtf = """
            {\\rtf1\\ansi Account Name: X\\uc0\\u8232 Secret Key: GEZDGNBVGY3TQOJQ\\u8232 \
            Hash Algorithm: sha1\\u8232 \(present)\\u8232 }
            """
        let result = LabelledTextImport.read(rtf)

        #expect(result.accounts.isEmpty, "an incomplete record must not be imported")
        #expect(result.refusals.first?.reason == expected)
    }

    @Test("A missing algorithm is refused too")
    func refusesMissingAlgorithm() {
        let rtf = """
            {\\rtf1\\ansi Account Name: X\\uc0\\u8232 Secret Key: GEZDGNBVGY3TQOJQ\\u8232 \
            Digits: 6\\u8232 Period: 30 seconds\\u8232 }
            """
        let result = LabelledTextImport.read(rtf)
        #expect(result.refusals.first?.reason == .missingSetting(.algorithm))
    }
