...
| **The Keychain access group deliberately does not match the bundle identifier** | group `com.openfactor.shared`, identifiers `dev.openfactor.app…` | The identifiers were renamed when it became clear that `com.openfactor.dev` claimed a domain this project does not own, since reverse DNS of `openfactor.dev` is `dev.openfactor`. **The access group was deliberately left alone.** It is an arbitrary name that only has to be stable, it is not derived from the bundle identifier, and every account on every device and in iCloud Keychain lives inside it. Renaming it for tidiness would strand all of them behind a migration whose failure mode reads as "my accounts vanished". Verified after the rename by reading the entitlement out of the signed binary: `HST4KH9P2X.com.openfactor.shared`, unchanged. If you are here because the mismatch looks like an oversight, it is not |
| `CODE_SIGN_ENTITLEMENTS` | `OpenFactor/OpenFactor.entitlements` | Declares one shared Keychain access group, `$(AppIdentifierPrefix)com.openfactor.shared`. The watch app has its own bundle identifier and would otherwise look in its own group and find nothing. The group is named only here: the app writes without specifying one, and the Keychain uses the first entitlement group as the default, so the group name is never written down twice |
